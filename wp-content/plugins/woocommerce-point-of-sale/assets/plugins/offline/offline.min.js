...
jQuery('document').ready(function ($) {
    Offline.options = {
        checks: {
            xhr: {
                url: function () {
                    return wc_pos_params.offline_url + "?_=" + (Math.floor(Math.random() * 1000000000));
                },
                timeout: 5000
            },
            image: {
    ...
    Offline.on('confirmed-down', function () {
        ...
        if( $indication.hasClass('offline-ui-up') ){
         closeModal();
         openModal('modal-lost-connection');
         }
        $indication.removeClass('offline-ui-up').addClass('offline-ui-down');
        jQuery('.offline_counter').css("display", "inline-block");
        jQuery('#offline_counter').append('<span id="offline_count"></span>');
        APP.db.keys('offline_orders').then(function (keys) {
            jQuery('#offline_count').text(count(keys));
        })
    });

    Offline.on('confirmed-up', function () {
        if ($indication.hasClass('offline-ui-down')) {
            closeModal();
            openModal('modal-reconnected-successfuly');
            console.log('Offline mode off');
            process_offline_orders();
        }
        $indication.removeClass('offline-ui-down').addClass('offline-ui-up');
        jQuery('#offline_counter').css("display", "none");
    });
