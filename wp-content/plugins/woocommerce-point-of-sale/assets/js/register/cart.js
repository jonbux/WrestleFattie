jQuery(document).ready(function ($) {
    ...
    window.POS_CART = {
        ...
        /** @var object */
        pos_refund : {refund: false, order: null, items: []},
        ...
        addToCart: function (product_data, product_id, quantity, variation_id, variation, cart_item_data) {
            ...
            try {
                ...
                if (cart_item_key) {
                    ...
                    $('.tbc.tab.active #order_items_list').append(html);
        ...
        empty_cart: function (updateCastomer) {
            $.each(CART.cart_empty_data, function (key, _default) {
                CART[key] = clone(_default);
            });
            //TODO: need to find another way
            //reset fees
            CART.cart_session_data.fees = {};
            $('tr.fee_row').remove();
            ...
            if (wc_pos_params.tabs_management) {
                $('.tbc.tab.active #order_items_list').html('');
                $('#order_comments').val('');
            } else {
        ...
        calculate_totals: function () {
                ...
                $.each(CART.fees, function (index, fee) {
                    ...
                    if (fee.taxable) {
                        ...
                        $.each(taxes, function (i, t) {
                            if(wc_pos_params.pos_calc_taxes == true){
                                CART.taxes[i] = CART.taxes[i] ? CART.taxes[i] + floatval(t) : floatval(t) ;
                            }else{
            ...
            //resizeCart();
            if (wc_pos_params.tabs_management) {
                jQuery('.box-tab[data-tab_id="' + APP.active_tab + '"] .opened-amount').text(currency_symbol + CART.total.toFixed(2));
            }
        },
        /**
         * Sort by subtotal
        ...
        needs_shipping: function () {
            ...
            var shipping_price = parseFloat(this.chosen_shipping_methods.price);
         ...
         */
        add_custom_discount: function (amount, type, coupon_code, text) {
            ...
            var the_coupon = new WC_Coupon(coupon_code, {'type': type, 'amount': amount, 'text': text});
            ...
            if (wc_pos_params.tabs_management) {
                APP.tabs[APP.active_tab].setCart('coupons', CART.coupons);
                APP.tabs[APP.active_tab].setCart('applied_coupons', CART.applied_coupons);
            }
            CART.calculate_totals();
        },
        ...
        add_custom_fee: function (fee) {
            ...
            CART.calculate_totals();
            if (wc_pos_params.tabs_management) {
                APP.tabs[APP.active_tab].setCart('fees', CART.fees);
                APP.tabs[APP.active_tab].setCart('fee_total', CART.fee_total);
                APP.tabs[APP.active_tab].setCart('cart_session_data', CART.cart_session_data);
            }
        },
        ...
        calculateAmountTendered: function () {
            ...
            $.each(tendered_num, function (key, val) {
                if (val == '') {
                    ...
                    jQuery('.amount_pay_keypad .keypad-' + key).text(currency_symbol + val).show();
                }
            });

        },
        setup_refund: function(refund_data){
            var $this = this;
            $this.pos_refund = refund_data;
            APP.db.get('orders', $this.pos_refund.order.id).done(function(order){
                var original_order = order;
                APP.db.put('orders', $this.pos_refund.order);
                APP.loadOrder(order.id);
                APP.db.put('orders', original_order);
                APP.setup_refund_elements();
            });
        }
