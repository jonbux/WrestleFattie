    ...
    window.POS_PR_ADDONS = {
        init: function () {
            ...
            $cart.on('click', '#reset_selected_variation', function () {
                ...
                $.each($cart.find('.product-fields-table'), function () {
            ...
            $cart.on('wc_pos_updated_addons', function () {
                ...
                var totals_element = $("table#selected-variation-data"),
                    bottomButtonBar = $(".md-register .md-content > div.wrap-button"),
                    optionsDiv = $("#modal-missing-attributes .md-content > div:not(.wrap-button)");

                if(totals_element.length && bottomButtonBar.length && optionsDiv.length){
                    var bottomTotal = intval(bottomButtonBar.outerHeight() + totals_element.outerHeight() + 10);
                    optionsDiv.css({bottom: bottomTotal + 'px !important'});
                }
            });
                    ...
                    $.each(product_addons, function (index, addon) {
                        ...
                        switch (addon.type) {
                            ...
                            case 'multiple_choice':
                                ...
                                if ($input.length && $input.val() != '') {
                                    fields[0] = $('.' + addon.type + '_' + index).find(":selected").text().trim();
                                }
                                break;
                        }
        ...
        validate_missing_attributes: function (missing, adding_to_cart, product_id, quantity, variation_id, selected_attr, cart_item_data) {
            ...
            if (product_addons.length > 0) {
                ...
                if (missing) {
                    product_addons = product_addons.filter(function (addon) {
                        return addon.type !== 'heading';
                    });
