        ...
        return this.each(function () {
            ...
            $_this.on('click', '#tile_go_back', function (event) {
                ...
                if (length > 1) {
                    ...
                    $('.open_variantion_attr').remove();
                    $('#wc-pos-register-grids-title').trigger('click');
                }
                parentList.find("li").first().before(parentList.find("#tile_go_back"));
            });
            ...
            $('#wc-pos-register-grids .inside').on('scroll', function () {
                if (this.scrollHeight - this.scrollTop === this.clientHeight) {
                    ...
                    var prodTitle = settings.breadcrumbs.find('.prod_title');
                    if(prodTitle.length && prodTitle.first().data('id') > 0){
                        return;
                    }
                    ...
                    $.each(records, function (index, val) {
                        if (typeof val != 'undefined') {
                            ...
                            if (typeof val.type != 'undefined' && val.type == 'variable' && val.variations.length > 0 && pos_grid.tile_variables == 'tiles') {
                                var $li = $('<li id="product_' + val.id + '" class="title_product open_variantion category_cycle" data-id="' + val.id + '" title="'+val.title+'"><span></span><span class="price"></span></li>');
                            } else {
                                var $li = $('<li id="product_' + val.id + '" class="title_product add_grid_tile category_cycle" data-id="' + val.id + '" title="'+val.title+'"><span></span><span class="price"></span></li>');
                            }
        ...
        if (typeof POS_TRANSIENT.grid_product_variations == 'undefined' || typeof POS_TRANSIENT.grid_product_attributes == 'undefined') {
            ...
            window.POS_APP.db.get("products", parent).done(function (record) {
                ...
                $.each(record.variations, function (index, val) {
                    ...
                    var id = val.id ? val.id : index;
                    POS_TRANSIENT.grid_product_variations[id] = {};
                    POS_TRANSIENT.grid_product_variations[id]['attributes'] = attributes;
                    POS_TRANSIENT.grid_product_variations[id]['variation_is_active'] = true;
                    POS_TRANSIENT.grid_product_variations[id]['variation_id'] = val.id;
                    POS_TRANSIENT.grid_product_variations[id]['data'] = val;
                ...
                if (typeof current_settings[tax] == 'undefined' && !build) {
                    ...
                    $.each(options, function (index, val) {
                        if (in_array(val.slug, _options) || all_opt) {
                            variable_index = Object.keys(POS_TRANSIENT.grid_product_variations)[index];
                            var title = val.name,
                                price = '';
                            if (cs_count + 1 == pa_count && pos_grid.tile_layout == 'image_title_price') {
                                ...
                                if (variation) {
                                    ...
                                    variable_index = variation.variation_id;
                                }
                            }
            ...
            if (record && record.type == 'variable' && count > 0) {
                ...
                if (variations.length) {
                    $.each(variations, function (index, val) {
                        ...
                        var $li = $('<li id="product_' + val.id + '" class="title_product add_grid_tile category_cycle" data-id="' + parent + '" data-varid="' + val.id + '" title="'+val.title+'"><span></span></li>');
