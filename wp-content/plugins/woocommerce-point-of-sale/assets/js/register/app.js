...
jQuery(document).ready(function ($) {
    ...
    $(window).load(function(){
        $('#preloader').hide(0,function(){$(this).remove();});
    });
    ...
    var wc_pos_dining_option_default = wc_pos_dining_option;
...
// ajaxSetup is global, but we use it to ensure JSON is valid once returned.temp@_@admintemp
    ...
    APP = window.POS_APP = {
        ...
        sync_data: function (block) {
            ...
            if (wc_pos_params.disable_customers_cache == 'no') {
            ...
            if (customer_id != '' && parseInt(customer_id) > 0) {
                ...
                if (wc_pos_params.disable_customers_cache == 'no') {
                    APP.db.get('customers', customer_id).done(function (record) {
                        if ( record.user_meta && typeof record.user_meta.wp_user_avatar != 'undefined' && record.user_meta.wp_user_avatar[0]) {
                            $.ajax({
                                type: 'POST',
                                url: wc_pos_params.ajax_url,
                                data: {
                                ...
                                success: function (response) {
                                    ...
                                    if (wc_pos_params.tabs_management) {
                                        APP.tabs[APP.active_tab].setCustomer(CUSTOMER);
                                    }
                    ...
                    $.ajax({
                        url: wc_pos_params.wc_api_url + 'customers/' + customer_id,
                        success: function (response) {
                            if (typeof response.customer.user_meta.wp_user_avatar != 'undefined' && response.customer.user_meta.wp_user_avatar[0]) {
                                $.ajax({
                                    type: 'POST',
                                    url: wc_pos_params.ajax_url,
                                    data: {
                                ...
                                CUSTOMER.set_default_data(response.customer);
                                APP.setUpCustomerModal(wc_pos_params.load_customer, true);
                            }
                        }
                    });
        ...
        paymentProcessBeforeCheckout: function(){
            var selectedPM = $('.select_payment_method:checked:not(:disabled)').val(),
        ...
        processRefundAmountBeforeRefunds: function(){
            var refundOrder = POS_CART.pos_refund.order,
                deferredObject = $.Deferred();
            if(!refundOrder || empty(wc_pos_params.payment_sense_url) || refundOrder.payment_details.method_id !== "wc_pos_paymentsense"){
                deferredObject.reject();
                return deferredObject.promise();
            }
        ...
        createOrder: function (paid, paymentSense) {
            ...
            var cart = {
                order: {
                    ...
                    "order_meta": {
                        ...
                        "wc_pos_dining_option": wc_pos_dining_option,
                        "wc_pos_signature": APP.signature[1],
            ...
            if(!$.isEmptyObject(paymentSense)){
                cart.order.paymentSense = paymentSense;
            }

            if($("#payment_switch").bootstrapSwitch('state')){
                cart.order.print_receipt = true;
            }
            ...
            if (wc_pos_params.tabs_management) {
                cart.order.order_meta['order_tab'] = jQuery('.woocommerce_order_items_wrapper .tab.active').data('tab_number');
            }
            ...
            $.each(wc_pos_params.acf_order_fields, function (index, key) {
                ...
                if($el.length){
                    ...
                    cart.order.custom_order_meta[key] = _val;
                }
            });
            ...
            if (paid) {
                ...
                cart.order.order_meta.wc_pos_dining_option = wc_pos_dining_option;
                cart.order.order_meta.wc_pos_signature = APP.signature[1];
            } else {
        ...
        create_refund: function(){
            ...
            var wc_api_url = wc_pos_params.wc_api_url + 'pos_orders/' + CART.pos_refund.order.id + "/refunds";
            wc_api_url += '/?v=' + APP.makeid();

            $.each(CART.pos_refund.order.line_items, function(i, item){
                CART.pos_refund.order.line_items[i].qty = Math.abs(item.qty);
            });

            CART.pos_refund.order["amount"] =  Math.abs(CART.total) ;
            CART.pos_refund["order_refund"] = CART.pos_refund.order;
            CART.pos_refund["order_refund"]["restock_items"] = $("#reduce_stock").is(":checked");
            ...
            $.ajax({
                ...
                processData: false,
                data: JSON.stringify(CART.pos_refund),
                success: function(response){
                    if(!response){
                        ...
                        modalConfirm.on("click", "#confirm-button", function(){
                            closeModal("modal-confirm-box");
                            posPrintReceipt(response.order_refund.refund_receipt);
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (paid) {
                        if (typeof data != 'undefined' && typeof data['payment_result'] != 'undefined' && typeof data['payment_result']['result'] != 'undefined') {
                            if (data['payment_result']['result'] == 'error') {
                                ...
                                if (typeof data['payment_result']['redirect'] != 'undefined' && data['payment_result']['redirect'] != '') {
                                    ...
                                    if ($('#payment_switch').bootstrapSwitch('state')) {
                                        if (print_receipt != 0 && (!isValidMac(receiptPrinter) || data['order']['print_type'] === "html")) {
                        ...
                        if (wc_pos_params.tabs_management) {
                            if (jQuery('.woocommerce_order_items_wrapper .tab.active').data('order_id') == data.order.id) {
                                let tab_data = {
        ...
        ready: function () {
            ...
            if ($('#customer_user').length) {
                if (wc_pos_params.disable_customers_cache == 'no') {
                    ...
                    $('#customer_user').select2({
                        ...
                        ajax: {
                            url: function (params) {
                                return wc_pos_params.wc_api_url + 'customers/?filter[q]=*' + params.term + '*'
                            },
                            dataType: 'json',
                            delay: 500,
            ...
            $(document).on('click', 'a.show_customer_popup', function (event) {
                ...
                $('#billing_state').val(CUSTOMER.billing_address.state).trigger('change');
                $('#shipping_state').val(CUSTOMER.shipping_address.state).trigger('change');
                ...
                $.when(APP.paymentProcessBeforeCheckout()).then(function(response){
                    ...
                    if(error.showPaymentMethods){
                        ...
                        if(error.responseJSON){
                            if(error.responseJSON.messages && error.responseJSON.messages.error){
        ...
        setUpCustomerModal: function(_openModal, setAccountFunds, event){
            ...
            $('#billing_state').val(CUSTOMER.default_state).trigger('change');
            $('#shipping_state').val(CUSTOMER.default_state).trigger('change');
            if(CUSTOMER.account_funds && !empty(CUSTOMER.account_funds)){
                $("#account-funds-data").children('strong').html(CUSTOMER.account_funds);
            }else{
            ...
            modal_order_customer.on("change", "#billing_country, #shipping_country", function () {
                ...
                $.ajax({
                    ...
                    success: function(response){
                        $("#" + type + "_state_field").remove();
                        $(response.data).find('select').select2();
                        $("#"+type+"_city_field").after($(response.data));
                        $("select#"+type+"_state").select2({
        ...
        checkPOSUserLogin : function(data){
            ...
            location.href = wc_pos_params.admin_url + "admin.php?page=wc_pos_registers&close=" + pos_register_data.ID;
        },
    ...
    // $(document).on('click', '#select2-customer_user-results > li', function (e) {
    //     $('a.show_customer_popup').trigger('click');
    // });
