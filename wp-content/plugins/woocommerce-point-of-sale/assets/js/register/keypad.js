...
jQuery(document).ready(function ($) {
    ...
    calculateSelectedPrice = function () {
        ...
        $('#order_items_list .new_row .product_price').keypad({
            ...
            beforeShow: function (div, inst, value) {
                ...
                jQuery('#discount-value').keydown(function(e){
                    if(e.which === 13){
                        inst._mainDiv.find('.keypad-close').click();
                    }
                }).on('keyup', function(e){
                    ...
                    if($(".disc-type.active").hasClass('percents')){
                        var clickedVal = $(this).val(),
                            row_id = inst.elem.data('row'),
                            cur_value = CART.cart_contents[row_id].original_price ? CART.cart_contents[row_id].original_price :  CART.cart_contents[row_id].price;

                        inst.elem.val(cur_value - (cur_value * clickedVal / 100));
                    }else{
                        inst.elem.val($(this).val());
                    }
                }).select();
                hideEmptyDiscounts();
            },
            onClose: function (value, inst) {
                var val_input = $('#discount-value');
                var row_id = val_input.data('row');
                var price_input = $('#item_note-' + row_id).find('.product_price');
    ...
    $.keypad.addKeyDef('FIVEPERCENT', 'fivepercent', function (inst) {
        ...
        if(inst.elem.hasClass('product_price')){
            doProductPrice(inst);
        }
    ...
    function doProductPrice(instant){
        if(!$('.keypad-popup').is(':visible')){
            return;
        }

        var clickedVal = instant.elem.val(),
            discountInput = $("#discount-value"),
            row_id = instant.elem.data('row'),
            cur_value = CART.cart_contents[row_id].original_price ? CART.cart_contents[row_id].original_price :  CART.cart_contents[row_id].price;

        discountInput.val(clickedVal);
        instant.elem.val(cur_value - (cur_value * clickedVal / 100));
    }

    jQuery('body').on('click', '.restore-price', function () {
        var row_id = jQuery('#discount-value').data('row');
        jQuery('#discount-value, .product_price[data-row=' + row_id + ']').val(CART.cart_contents[row_id].original_price);
    });
});
