...
class WC_Pos_Sell
{
    ...
    public $data = null;
    public $script_ver = '4.2.8.7';
    public $id = null;
    public $active_plugins = array();
    private $req_scripts = array();
...
#	        add_action( 'wp_login', array( $this, 'set_last_login') );
            ...
            add_filter('script_loader_tag', array($this, 'remove_async_script_attribute'), 99, 2);
        }
    }
    ...
    public function init_addons_hooks()
    {
        ...
        if (class_exists('WC_Product_Addons') && get_option('wc_pos_force_enable_addons', 'wc_pos_force_enable_addons') == "yes") {
        ...
        add_action('wc_pos_footer_scripts', array($this, 'footer'));
        add_action('pos_admin_print_scripts', array($this, 'header'),15);
            ...
            if( $acfs ) {
                foreach( $acfs as $acf ) {
                    ...
                    foreach ($fields as $field) {
                        $checkout_fields['pos_custom_order']['acf-field-'.$field['name']] = get_merged_acf_array($field);
                    }
    ...
    public function template_redirect()
    {
        ...
        if (is_user_logged_in()) {
            ...
            if (get_option('wc_pos_tabs_management', 'no') == 'yes') {
                $data['tabs'] = get_tabs($data['ID']);
                $tab_default_limit = get_option('wc_pos_tab_default_spend_limit', 0);
            }
    ...
    public function validate()
    {
        ...
        if ($user_id = pos_check_register_lock($this->id)) {
            ...
            $is_force_open = get_option('wc_pos_force_logout', 'no');
            ?>
            <div class="md-modal md-dynamicmodal md-message md-openmodal" id="modal-locked-register">
                <div class="md-content">
                    <div>
                        <div class="post-locked-avatar"><?php echo get_avatar($user->ID, 64); ?></div>
                        <p tabindex="0">
                            <?php printf(__('%s has already opened this register.', 'wc_point_of_sale'), $user->display_name); ?>
                        </p>
                        <?php if($current_user->has_cap('pos_force_logout') && $is_force_open === "yes"): ?>
                        <p tabindex="0"><?php _e('You cannot use this register unless you take over.', 'wc_point_of_sale'); ?>
                        </p>
                        <?php endif; ?>
                        <a class="button" style="margin-right: 10px;"
                           href="<?php echo admin_url('admin.php?page=wc_pos_registers'); ?>"><?php _e('All Registers', 'wc_point_of_sale'); ?></a>
                        <?php if($current_user->has_cap('pos_force_logout') && $is_force_open === "yes"): ?>
                        <button class="button button-primary" id="force_open_reg"><?php _e('Take Over', 'wc_point_of_sale'); ?></button>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
    ...
    public function header()
    {
        ...
        $styles = array(
            ...
            'wc-pos-loadie' => WC_POS()->plugin_url() . '/assets/css/register/loadie.css',
        );
        $styles = apply_filters('wc_pos_enqueue_styles', $styles);

        foreach ($styles as $key => $style) {
            // .= str_replace(' ', '%20', $script) . "\n";
            echo "\n" . '<link media="all" type="text/css" href="' . $style . '" id="' . $key . '" rel="stylesheet">';
            //wp_enqueue_style($key, $style, array(), time());
    ...
    public function footer()
    {
        ...
        $scripts = array(
            ...
            'wc-pos-visible' => WC_POS()->plugin_url() . '/assets/js/register/plugins/jquery.visible.min.js',///todo: remove?
            ///
            ...
            'wc-pos-loadie' => WC_POS()->plugin_url() . '/assets/js/register/plugins/jquery.loadie.js',
        ...
        if (get_option('wc_pos_signature', 'no') == 'yes') {
            $scripts['wc-pos-signature'] = WC_POS()->plugin_url() . '/assets/js/register/plugins/jSignature.min.js';
        }

        if (get_option('wc_pos_tabs_management', 'no') == 'yes') {
            $scripts['wc-pos-timer-js'] = WC_POS()->plugin_url() . '/assets/js/register/plugins/timer/dist/timer.jquery.min.js';
            $scripts['wc_pos_tabs'] = WC_POS()->plugin_url() . '/assets/js/register/tab.js';
        }
        if (WC_VERSION >= 3) {
        ...
        if (get_option('wc_pos_keyboard_shortcuts') == 'yes') {
            $scripts['jquery-hotkeys-new'] = WC_POS()->plugin_url() . '/assets/plugins/jquery.hotkeys.js';
        ...
        if (get_option('wc_pos_enable_new_api', 'no') == 'no') {
            ...
            $scripts['wc-pos-ydn'] = WC_POS()->plugin_url() . '/assets/js/register/plugins/ydn.db-dev.js';
            $scripts['wc-pos-category_cycle'] = WC_POS()->plugin_url() . '/assets/js/register/category_cycle.js';
            $scripts['wc-pos-functions'] = WC_POS()->plugin_url() . '/assets/js/register/functions.js';
            $scripts['wc-pos-cart'] = WC_POS()->plugin_url() . '/assets/js/register/cart.js';
            $scripts['wc-pos-init'] = WC_POS()->plugin_url() . '/assets/js/register/app.js';
            $scripts['handlebars-helpers'] = WC_POS()->plugin_url() . '/assets/js/register/handlebars/handlebars.helpers.js';

        } else {

            $scripts['wc-pos-category_cycle'] = WC_POS()->plugin_url() . '/assets/js/register/category_cycle-v1.js';
            $scripts['wc-pos-functions'] = WC_POS()->plugin_url() . '/assets/js/register/functions-v1.js';
            $scripts['wc-pos-cart'] = WC_POS()->plugin_url() . '/assets/js/register/cart-v1.js';
            $scripts['wc-pos-init'] = WC_POS()->plugin_url() . '/assets/js/register/app-v1.js';
            $scripts['handlebars-helpers'] = WC_POS()->plugin_url() . '/assets/js/register/handlebars/handlebars.helpers-v1.js';
...
//            echo "\n" . '<script src="' . $script . '?v=' . $v . '"></script>';
            wp_enqueue_script($name, $script, array('jquery'), time(), true);
        }

        $this->req_scripts = $scripts;
    ...
    public static function getJsParams()
    {
        ...
        $params = apply_filters('wc_pos_params', array(
            ...
            'enable_user_card' => (get_option('wc_pos_enable_user_card', 'no') == 'yes' ? true : false),
            'order_prompt' => (get_option('wc_pos_order_prompt', 'no') == 'yes' ? true : false),
            ...
            'tabs_management' => (get_option('wc_pos_tabs_management', 'no') == 'yes' ? true : false),
...
//            'wc_api_url' => home_url('wp-json/wc/v2/'),
            ...
            'signature_panel' => get_option('wc_pos_signature', 'no') == 'yes' ? true : false,
            'signature_required' => get_option('wc_pos_signature_required', 'no') == 'yes' ? true : false,
            ...
            'use_refund' => get_option('wc_pos_refund', 'no') == 'yes' ? true : false,
            'disable_customers_cache' => get_option('wc_pos_disable_customers_cache', 'no'),
        ...
        if(in_array('wc_pos_paymentsense', get_option('pos_enabled_gateways', array()))){
            $payment_sense_data = get_option('woocommerce_wc_pos_paymentsense_settings', array());
            if(count($payment_sense_data)){
                $params['payment_sense_url'] = isset($payment_sense_data['payment_sense_url']) ? set_url_scheme(esc_url($payment_sense_data['payment_sense_url']), "https") : "";
                $params['payment_sense_api_key'] = isset($payment_sense_data['payment_sense_api_key']) ? $payment_sense_data['payment_sense_api_key'] : "";
            }
        }
...
//        WC()->product_factory->get_product();
    ...
    public function getGrid($is_api = false)
    {
        ...
        $tile_styles = array();
        $products_sort = array();
        $pos_visibility = get_option('wc_pos_visibility', 'no');
        if ($grid_id != 'all' && $grid_id != 'categories') {
            ...
            if (count($tiles)) {
                foreach ($tiles as $key => $style) {
                    $products = array();
                    if($style->style == "auto"){
                        $cat = get_term($style->product_id, "product_cat");
                        ...
                        foreach ($products_posts as $product){
                            ...
                            array_push($products, $wc_product);
                        }
                    }else{
                        array_push($products, $style);
                    }

                    foreach ($products as $i => $product){
                        $_id = isset($product->product_id) ? absint($product->product_id) : $product->get_id();
                        $add = true;
                        if ($out_of_stock != 'yes') {
                            if (get_post_meta($_id, '_stock_status', true) != 'instock') {
                                $add = false;
                            }
                        }
                        if ($pos_visibility == 'yes') {
    ...
    public function remove_async_script_attribute($script, $type)
    {
        $screen = function_exists('get_current_screen') ? get_current_screen() : null;
        if( $screen && $screen->id == "pos_page"){
            $script = str_replace( 'async="async"', '', $script );
        }
