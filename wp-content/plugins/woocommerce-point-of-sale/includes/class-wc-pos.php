...
//        add_action('init', array($this, 'email_fix_test'), 999);
    ...
    public function includes()
    {
        ...
        if (get_option('wc_pos_enable_new_api', 'no') == 'yes') {
            include_once('cache/class-wc-pos-cache.php');
            include_once('class-wc-pos-new-api.php');
        }
    ...
    public function init_hooks()
    {
        ...
        //add_action('init', array($this, 'check_pos_custom_product_exists'), 5);
        ...
        //add_action('init', array($this, 'set_user_api_key'), 10);
    ...
    public function set_user_api_key() {
        $api_keys = get_user_meta(get_current_user_id(), 'woocommerce_api_key', true);

        if (empty($api_keys)) {
            $adm = new WC_POS_Admin();
            $api_keys = $adm->create_api_key_for_user(get_current_user_id(), true );

        }

        if ($api_keys !== false) {
            if ( ! empty( $_GET['consumer_key'] ) && ! empty( $_GET['consumer_secret'] ) ) {
                $_GET['consumer_key'] = $api_keys['consumer_key'];
                $_GET['consumer_secret'] = $api_keys['consumer_secret'];
            }
        }
    }
