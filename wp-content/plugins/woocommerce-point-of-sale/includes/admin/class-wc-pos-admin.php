...
class WC_POS_Admin
{
    ...
    public function __construct()
    {
        ...
        add_action('add_meta_boxes', array(WC_Pos_Grids::instance(), 'add_meta_box'), 40, 1);
        add_action('save_post', array(WC_Pos_Grids::instance(), 'save_meta_box'), 40, 1);
    ...
    public function init_users_hooks()
    {
        ...
        add_action('edit_user_profile_update', array($this, 'create_api_key_for_user'));
    }
                ...
                foreach ($fieldset['fields'] as $key => $field) :
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <?php }elseif ($key == 'generate_wc_api_key'){
                                ...
                                if(!is_null($row)): ?>
                                    <input type="text" disabled value="..<?php echo $row['truncated_key'] ?>"><br>
                                    <a style="color: #a00; text-decoration: none;" href="<?php echo esc_url( wp_nonce_url( add_query_arg( array( 'revoke-key' => $row['key_id'] ), admin_url( 'admin.php?page=wc-settings&tab=advanced&section=keys' ) ), 'revoke' ) ) ?>"><?php _e("Revoke key", "wc_point_of_sale") ?></a>
                                <?php else: ?>
                                    <button class="button" id="generate_wc_key"><?php _e("Generate WooCommerce API Key", "wc_point_of_sale") ?></button>
                                    <input type="hidden" id="wc_api_nonce" value="<?php echo wp_create_nonce( 'update-api-key' ); ?>">
                                <?php endif; ?>
                            <?php }elseif(true){
    ...
    public function create_api_key_for_user( $user_id, $regenerate = false ) {
...
//        }
        ...
        if (empty($_POST['outlet']) AND ($regenerate == false)) {
            return;
        }
        ...
        try {
            ...
            $data = array(
                'user_id'         => $user_id,
                'description'     => 'POS API key for user ID#'.$user_id,
            ...
            update_user_meta( $user_id, 'woocommerce_api_key', $data );
    ...
    public function get_customer_meta_fields()
    {
        ...
        if(current_user_can('administrator')){
            $show_fields['outlet_filds']['fields']['generate_wc_api_key']  = array(
                'label'       => __( 'API', 'wc_point_of_sale' ),
                'type'        => 'button'
            );
        }
    ...
    function restrict_list_users()
    {
        ...
        if(get_current_screen()->id == "point-of-sale_page_wc_pos_users") unset($wc_pos_filters[1]);
    ...
    function wc_pos_outlets_filter()
    {
        ...
        <select id="dropdown_outlets" name="_outlets_filter">
            ...
            <?php foreach ($outlet_arr as $key => $value) : ?>
                <option value="<?php echo $key; ?>" <?php selected($outlet_id, $key, true); ?> ><?php echo $value ?></option>
            <?php endforeach; ?>
        </select>
        <?php
    }
    ...
    public function show_admin_bar_pos_registers($admin_bar)
    {
        ...
        $admin_bar->add_menu(array(
            ...
            'title' => '<span class="ab-icon"></span><span class="ab-label">' . __("Point of Sale", "wc_point_of_sale") . '</span>',
            'href'  => admin_url("admin.php?page=wc_pos_registers"),
            'icon' => 'asd',
            'meta'  => array(
                'title' => __('POS'),
                'icon' => 'asd'
        ...
        $admin_bar->add_menu(array(
            ...
            'title' => __("All registers", "wc_point_of_sale"),
            'href'  => admin_url("admin.php?page=wc_pos_registers"),
            'meta'  => array(
                'title' => __("All registers", "wc_point_of_sale"),
            ),
    ...
    public function wc_settings_button_field($value)
    {
        ...
        <th scope="row" class="titledesc">
            <label for=""><?php echo esc_html( $value['title'] ); ?> <?php echo $tooltip_html;?></label>
        </th>
