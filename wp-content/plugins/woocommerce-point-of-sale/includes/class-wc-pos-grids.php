...
class WC_Pos_Grids {
	...
	public function __construct() {
        ...
        add_filter('post_updated_messages', array($this, 'grid_notices'), 99, 1);
	}
	/**
		...
		if ( 'add' === $action || 'edit' === $action ) {
			...
			if ( ! empty( $error ) ) {
				...
				if ( 'add' === $action ) {
					...
					echo '<div id="message" class="notice notice-success"><p>Product Grid added successfully.</p></div>';
				...
				if ( 'edit' === $action ) {
					...
					printf('<div id="message" class="notice notice-success"><p><strong>Product grid updated.</strong></p><p><a href="%1$s">%2$s</a></p></div>',
                        add_query_arg(array("page" => WC_POS()->id_grids), admin_url('admin.php')),
                        __('&larr; Back to Product Grids', 'wc-point-of-sale')
                    );
					$action_completed = true;
				}
				flush_rewrite_rules();
            ...
            echo '<div id="message" class="notice notice-success"><p>Product Grid deleted successfully.</p></div>';
        }
    ...
    public function grid_notices($messages)
    {
        return $messages;
    }
