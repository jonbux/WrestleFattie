...
class WC_Pos_API
{
    ...
    public function init_hooks()
    {
        ...
        if ( get_option( 'wc_pos_enable_new_api', 'no' ) == 'no' ) {
            ...
            add_filter( 'woocommerce_api_product_response', array( $this, 'filter_product_response' ), 99, 4 );
    ...
    public function filter_customer_response($customer_data, $customer, $fields, $server)
    {
        ...
        if(class_exists('WC_Gateway_Account_Funds') && in_array('accountfunds', get_option('pos_enabled_gateways', array()))){
            $id = isset($customer_data['id']) && $customer_data['id'] > 0 ? $customer_data['id'] : 0;
            $customer_data['account_funds'] = WC_Account_Funds::get_account_funds($id);
        }
    ...
    public function filter_order_response($order_data, $the_order, $fields, $api)
    {
        ...
        if (sizeof($order_data['line_items']) > 0) {
            foreach ($order_data['line_items'] as $key => $item) {
                ...
                $order_data['line_items'][$key]['image'] = wp_get_attachment_image(get_post_thumbnail_id($item['product_id']));
