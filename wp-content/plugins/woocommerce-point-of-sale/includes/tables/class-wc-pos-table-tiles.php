...
class WC_Pos_Table_Tiles extends WP_List_Table
{
    ...
    function column_thumb($item)
    {
        ...
        }elseif (has_post_thumbnail($id_pr)) {
    ...
    function column_product($item)
    {
        ...
        if($item['style'] == "auto"){
            return sprintf('%1$s %2$s', '<strong>' . __("Tiles will be dynamic and automatic load from this category", "wc_point_of_sale") . '</strong>', $this->row_actions($actions));
        }
