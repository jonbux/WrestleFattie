...
class WC_Pos_Table_Sessions extends WP_List_Table
{
    ...
    function get_columns()
    {
        $columns = array(
            ...
            'id' => __('ID', 'wc_point_of_sale'),
            'register_id' => __('Register ID', 'wc_point_of_sale'),
    ...
    {
        return sprintf(
            '<input type="checkbox" name="id[]" value="%s" />', isset($item['id']) ? $item['id'] : 0
        );
    }
    ...
    function column_register_name($item)
    {
        $register = $item['register_name'];
        return $register . '<br/>' . $item['outlet_name'];
    }
