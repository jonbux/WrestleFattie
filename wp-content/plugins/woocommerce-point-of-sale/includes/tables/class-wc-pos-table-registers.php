...
class WC_Pos_Table_Registers extends WP_List_Table
{
    ...
    function get_columns()
    {
        $columns = array(
            ...
            'paymentsense_terminal' => '<span class="terminal_head tips" data-tip="' . esc_attr__('Terminal', 'wc_point_of_sale') . '">' . esc_attr__('Terminal', 'wc_point_of_sale') . '</span>',
        ...
        if(!in_array("wc_pos_paymentsense", get_option('pos_enabled_gateways', array()))){
            unset($columns['paymentsense_terminal']);
        }
        return $columns;
    }
    ...
    function column_name($item)
    {
        ...
        if ($outlets_name[$item['outlet']] && pos_check_user_can_open_register($item['ID']) && !pos_check_register_lock($item['ID']) && WC_POS()->wc_api_is_active) {
            ...
            $name = sprintf(
                '<strong><a href="%s">%s</a></strong>', $register_url, $item['name']
            );
    ...
    function column_access($item)
    {
        ...
        if (pos_check_user_can_open_register($item['ID']) && !pos_check_register_lock($item['ID']) && WC_POS()->wc_api_is_active) {
            ...
            if (!WC_POS()->wc_api_is_active) {
                ...
                $btn_text = __('Open', 'wc_point_of_sale');
                $classes = "button tips open-register";
                $attributes = "disabled";
                if ($user) {
                    ...
                    if($current_user->has_cap('pos_force_logout') && $is_force_open === "yes"){
                        $btn_text = __('Open', 'wc_point_of_sale');
                        $classes .= " button-primary opened-register";
                        $attributes = "href='" . $register_url . "'";
                    }

                    return '<a class="'.$classes.'" data-tip="' . $name . ' is currently logged on this register." '.$attributes.'>' . $btn_text . '</button>';
                } else {
                    return '<a class="'.$classes.'" data-tip="You are not assigned to this outlet" '.$attributes.'>' . $btn_text . '</button>';
                }
            }
        }
    }
    ...
    function column_paymentsense_terminal($item){
        $terminal = isset($item["detail"]["paymentsense_terminal"]) ? $item["detail"]["paymentsense_terminal"] : __("None", "wc_point_of_sale");

        return ucfirst($terminal);
    }
    ...
    public function get_views()
    {
        ...
        if ($closed) {
            $views['close'] = '<a href="' . admin_url('admin.php?page=wc_pos_registers&status=close') . $url . '" class="' . $class . '" aria-current="page">' . __('Closed') . ' <span class="count">(' . $closed . ')</span></a>';
        }
        return $views;
    }
