    ...
    class WC_Pos_Session_Reports
    {
        ...
        public function __construct($register_data = '', $cashier_id = 0)
        {
            if ($register_data) {
                ...
                $this->session_data['report_data']['opening_cash_amount'] = isset( $detail->opening_cash_amount ) ? $detail->opening_cash_amount : 0;
                $this->session_data['report_data']['cash_management_actions'] = isset( $detail->cash_management_actions ) ? $detail->cash_management_actions : 0;
                $this->session_data['report_data']['actual_cash'] = isset( $detail->actual_cash ) ? $detail->actual_cash : 0;
        ...
        public function get_data($page = 1, $per_page = 50)
        {
            ...
            foreach ($db_data as $key => $val) {
                ...
                $this->session_data[$key]['outlet_name'] = $outlet_name[$val->outlet_id];
