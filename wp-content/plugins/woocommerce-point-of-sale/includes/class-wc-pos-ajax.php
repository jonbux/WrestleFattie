...
class WC_POS_AJAX
{
    ...
    public function __construct()
    {
        ...
        $ajax_events = array(
            ...
            'save_tab' => false,
            'get_customer_html' => false,
            'delete_tab' => false,
            'check_user_card_uniqueness' => false,
            'get_user_by_card_number' => false,
            'rename_payment_gateway' => false,
            'logout' => false,
            'pos_print_receipt' => false,
            'use_store_address' => false,
            'test_print' => false,
            'clear_cashier_count_orders' => false,
            'get_states_country' => false,
            'force_register' => false,
            'clear_order_queue' => false,
            'clear_order_history' => false,
            'paymentsense_eod_report' => false
        );
...
// Store variation data in meta
    ...
    public function json_search_variation_pr($parent_id, $v_id)
    {
        ...
        $title .= '<a target="_blank" href="' . esc_url(admin_url('post.php?post=' . absint($id) . '&action=edit')) . '">' . esc_html($product->get_title()) . '</a>';
        $f_title .= esc_html($product->get_title());
        ...
        $found_products['pr_inc_tax'] = wc_get_price_including_tax($product);
        $found_products['pr_excl_tax'] = wc_get_price_excluding_tax($product);
        ...
        if ($posts) {
            foreach ($posts as $post) {
                ...
                if ($product->is_type('variable')) {
                    $variations = $product->get_available_variations();//todo: maybe deprecated
                ...
                $found_products[$id]['pr_inc_tax'] = wc_get_price_including_tax($product);
                $found_products[$id]['pr_excl_tax'] = wc_get_price_excluding_tax($product);
    ...
    public function change_stock()
    {
        ...
        $value = $_POST['value'];
        $note = __('Product ', 'wc_point_of_sale');
        ...
        if ($product_id) {
            ...
            if ($operation == 'increase') {
                $stock += $value;
                $note .=  '<strong>' . $product->get_name() . '</strong>' . __(' stock increased by ', 'wc_point_of_sale') . $value;
            } else {
                $stock -= $value;
                if ($stock < 0) {
                    $stock = 0;
                }
                $note .=  $product->get_name() . __(' stock reduced by ', 'wc_point_of_sale') . $value;
            }
            ...
            if ($product->get_type() == 'variation' && $product->get_parent_id() && $product->get_parent_id() > 0) {
                wp_update_post(array(
    ...
    public function get_grid_options()
    {
        ...
        $pos->getRegisted(intval(isset($_POST['reg']) ? $_POST['reg'] : 0));
        echo $pos->getGrid();
        die();
    }
    ...
    {
        global $wpdb;
        $table = $wpdb->prefix . 'wc_poin_of_sale_tabs';
        $data = array(
            'title' => $_POST['title'],
            'spend_limit' => $_POST['limit'],
            'register_id' => $_POST['reg_id'],
            'order_id' => $_POST['order_id'],
            'tab_number' => $_POST['tab_number']
        );
        if (!intval($_POST['tab_id'])) {
            $data['opened'] = date('Y-m-d H:i:s', time() - $_POST['seconds']);
            $result = $wpdb->insert($table, $data);
            wp_send_json($data);
        } else {
            $result = $wpdb->update($table, $data, array('id' => $_POST['tab_id']));
        }
        wp_send_json($result);
    }
    ...
    public function get_customer_html()
    {
        if ($_POST['customer_id']) {
            $customer = new WP_User($_POST['customer_id']);
            if ($customer) {
                $html = '<option value="' . $customer->ID . '">' . $customer->display_name . ' (' . $customer->user_email . '/  ' . $customer->first_name . ')</option>';
                wp_die($html);
            }
        }
    }

    public function check_user_card_uniqueness()
    {
        ...
        if (count($users) == 0) {
            wp_send_json_success(__('You can use this code', 'wc_point_of_sale'));
        } else {
            wp_send_json_error(__('Sorry, this code is already present', 'wc_point_of_sale'));
        }
    }

    public function get_user_by_card_number()
    {
        ...
        if (count($users) == 0) {
            wp_send_json_error(_('User not found', 'wc_point_of_sale'));
        } else {
    ...
    public function rename_payment_gateway() {

        if (update_option(esc_attr($_POST['name']).'_name', esc_attr($_POST['value']))) {
    ...
    {
        $register_id = (isset($_POST['register_id'])) ? $_POST['register_id'] : 0 ;
        if ($register_id) {
    ...
    public function pos_print_receipt()
    {
        $data = array(
            'status_code' => 200,
            'message' => '',
        ...
        $status = get_option("wc_pos_enable_cloud_print", "disable");
        if($status === "disable"){
            $data['message'] = __("Point of Sale printing is disabled. Please enable printing to print receipts.", "wc_point_of_sale");
            $data['status_code'] = 400;

            wp_send_json_error($data);
        }

        $order_id = isset($_POST['pos_order_id']) && intval($_POST['pos_order_id']) ? intval($_POST['pos_order_id']) : 0;
        ...
        if(!isset($register[0])){
            $data['message'] = __("No register data found", "wc_point_of_sale");
            $data['status_code'] = 400;
        ...
        $printer = !empty($register[0]['settings']['receipt_printer']) ? $register[0]['settings']['receipt_printer'] : "show_print_dialogue";
        if(!WC_Pos_Cloud_Print_Handler::is_valid_mac($printer)){
            $data['message'] = __("Print window will be opened", "wc_point_of_sale");
            $data['status_code'] = 200;
            $data['print_url'] = wp_nonce_url(get_admin_url() . "?print_pos_receipt=true&order_id=$order_id");

            wp_send_json_success($data);
        }

        $data['message'] = __("Receipt sent to printer.", "wc_point_of_sale");
        $data['status_code'] = 200;
        $data['by_printer'] = true;
        WC_Pos_Cloud_Print::wc_pos_woo_on_thankyou($order->get_id());

        wp_send_json_success($data);
    }

    public function use_store_address()
    {
        global $woocommerce;
        $wc_country = $woocommerce->countries;
        $address = array(
            'country' => $wc_country->get_base_country(),
            'address_1' => $wc_country->get_base_address(),
            'address_2' => $wc_country->get_base_address_2(),
            'city' => $wc_country->get_base_city(),
            'state' => $wc_country->get_base_state(),
            'postcode' => $wc_country->get_base_postcode(),
    ...
    public function filter_order_barcode()
    {
        if(!isset($_POST['barcode']) || empty($_POST['barcode'])){
            wp_send_json_error(__('invalid order id', 'wc_point_of_sale'), 400);
        }

        $order = wc_get_order($_POST['barcode']);
        if(!$order){
            wp_send_json_error(__('order not found', 'wc_point_of_sale'), 404);
        }

        $invalid_statuses = array("refunded", "cancelled", "failed");
        if(in_array($order->get_status(), $invalid_statuses)){
            wp_send_json_error(__('order is already refunded', 'wc_point_of_sale'), 404);
        }

        $products = array();
        foreach ($order->get_items() as $id => $item){
            $product_data = $item->get_data();
            $product_data["image"] = wp_get_attachment_image(get_post_thumbnail_id($product_data['product_id']));
            $product_data["order_date"] = $order->get_date_created()->date_i18n();
            array_push($products, array_merge($product_data, wc_get_product($product_data['product_id'])->get_data()));
        }
    ...
    {
        $file = STAR_CLOUDPRNT_PRINTER_PENDING_SAVE_PATH.WC_POS_CPI()->star_cloudprnt_get_os_path("/order_TEST1234_".time().".bin");
        $printer_data = array();
        foreach (WC_POS_CPI()->star_cloudprnt_get_printer_list() as $key => $printer){
            if(get_option('wc_pos_selected_printer', '') == $key){
        ...
        if(!count($printer_data)){
            wp_send_json_error(__("Printer data not found", "wc_point_of_sale"), "404");
        }

        if(!$printer_data["printerOnline"]){
            wp_send_json_error(__("Printer is offline", "wc_point_of_sale"), "404");
        }

        $outlets = WC_POS()->outlet()->get_data_names();
        $outlet_name = "";
        $s_outlet = $printer_data['printerLocation'];
        ...
        $printer = new WC_Pos_Cloud_Print_Job($printer_data["printerMAC"], $file);
        $printer->set_codepage("20");
        $printer->add_text_line(__("Receipt Printer Test Page", "wc_point_of_sale"));
        $printer->add_new_line(1);
        $printer->add_new_line(1);
        $printer->add_text_line(__("MAC Address: ", "wc_point_of_sale") . $printer_data["printerMAC"]);
        $printer->add_text_line(__("Site Address: ", "wc_point_of_sale") . get_site_url());
        $printer->add_text_line(__("Printer Name: ", "wc_point_of_sale") . $printer_data["name"]);
        $printer->add_text_line(__("Printer Location: ", "wc_point_of_sale") . $outlet_name);
        $printer->add_text_line(__("IP Address: ", "wc_point_of_sale") . $printer_data['ipAddress']);
        $printer->add_new_line(1);
        $printer->add_text_line(__("WooCommerce Point of Sale", "wc_point_of_sale"));
        $printer->add_new_line(1);
        $printer->add_text_line(__("By Actuality Extensions", "wc_point_of_sale"));
        $printer->add_new_line(1);
        $printer->printjob(1);

        wp_send_json_success(__("A test print has been sent to the printer.", "wc_point_of_sale"));

    }

    public function clear_cashier_count_orders()
    {
        $updated = update_user_meta(get_current_user_id(), 'wc_pos_count_orders', 0);
        wp_send_json_success($updated);
    }

    public function get_states_country()
    {
        $country = isset($_POST['country']) ? $_POST['country'] : '';
        $type =  isset($_POST['type']) ? $_POST['type'] : 'billing';
        $fields = WC()->countries->get_address_fields($country, $type . '_');
        $fields = $fields['billing_state'];
        $fields['country'] = $country;
        $fields['return'] = true;

        wp_send_json_success(woocommerce_form_field($type . '_state', $fields));
    }

    public function force_register()
    {
        if(!isset($_POST['register_id']) || empty($_POST['register_id'])){
            wp_send_json_error("register not found", 404);
        }

        $response = pos_set_register_lock($_POST['register_id']);
        wp_send_json_success($response);
    }

    public function clear_order_queue(){
        $mac = isset($_POST['printerMac']) ? $_POST['printerMac'] : "";
        if(empty($mac)){
            wp_send_json_error("printer not found", 404);
        }

        WC_POS_CPI()->star_cloudprnt_queue_clear_list($mac);

        wp_send_json_success("printer queue cleared");
    }

    public function clear_order_history()
    {
        $history_path = STAR_CLOUDPRNT_DATA_FOLDER_PATH.WC_POS_CPI()->star_cloudprnt_get_os_path("/order_history.txt");
        if (file_exists($history_path))
        {
            $fh = fopen($history_path, 'w');
            fclose($fh);

            wp_send_json_success($fh, 200);
        }

        wp_send_json_error("history not found", 404);
    }

    public function paymentsense_eod_report()
    {
        $terminal_id = isset($_POST['terminal_id']) ? $_POST['terminal_id'] : 0;
        if(empty($_POST['terminal_id']) || $terminal_id == "none"){
            wp_send_json_error(array(
               "message" => 'invalid terminal id'
            ), 400);
        }

        $message = __('no data found', 'wc_point_of_sale');
        $payment_sense = new WC_Pos_PaymentSense_API();
        $request = $payment_sense->pac_reports($terminal_id, 0, array(
            'method' => 'POST',
        ...
        if(!is_wp_error($request)){
            ...
            if(isset($body->requestId)){
                ...
                if(isset($report_body->balances)){
                    ...
                    include trailingslashit(WC_POS()->plugin_path()) . "includes/admin/views/html-paymentsense-report.php";
