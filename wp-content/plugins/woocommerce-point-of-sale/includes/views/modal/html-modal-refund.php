        ...
        jQuery(document).ready(function($){
            ...
            $("#load_refund_products").on('click', function (e) {
                ...
                $("#barcode_options_table tbody tr").each(function(i, el){
                    ...
                    qty_count += parseInt(qty);
                    item_data.quantity = qty;
                    item_data.total = item_data.price * qty;
                    item_data.total_tax = (item_data.total_tax / r_qty) * qty;
                ...
                APP.db.get('orders', order_id).done(function(order){
                    if(!order){
                        APP.showNotice("This order cant be refundable", "error");
                        return;
                    }
                    var items_array = {};
                    $.each(order_items, function(i, item){
                        $.each(order.line_items, function(index, line_item){
                            if(item.product_id == line_item.product_id){
                                if(item.quantity > 0 ){
                                    var negative_qty = item.quantity - (item.quantity * 2);
                                    line_item["qty"] = negative_qty;
                                    line_item["quantity"] = negative_qty;
            ...
            function filter_order(order_id){
                ...
                APP.db.get('orders', order_id).done(function(order){
                    if(order){
                        ...
                        $.when(get_orders(order_id)).then(function (response) {
                            if( response.orders.length > 0 ) {
                                APP.db.putAll('orders', response.orders).fail(function (e) {
                                    throw e;
                                });
                                update_sku_controller_table (response.orders[0]);
                                $("#order_id").val(order_id);
                            } else {
