...
/*
 * Remove the ACF filter to prevent error throw by it
 */
wc_crm_remove_filters_for_anonymous_class( 'acf/input/admin_head', 'acf_controller_input', 'input_admin_head', 10 );
...
<!--<![endif]-->
<head>
	<meta http-equiv="Content-Type" name="viewport" charset="<?php echo get_option('blog_charset'); ?>" content="width=device-width, user-scalable=no, initial-scale=1, maximum-scale=1">
	<?php wp_site_icon() ?>
	...
	<style>
		@media only screen and (min-width: 1024px) {
			...
			.dining-option-selector.checked:after,
			#wc-pos-actions a.button:hover {
		...
		#modal-order_customer .wrap-button input:checked + label.pos_register_toggle,
        #modal-refund .wrap-button input:checked + label.pos_register_toggle{
			background: #b71c1c !important;
			background: <?php echo get_option('woocommerce_pos_register_base_color') ?> !important;
		}
		.loadie {
			background-color: #b71c1c !important;
			background-color: <?php echo get_option('woocommerce_pos_register_base_color') ?> !important;
		}
	</style>
	<?php
...
$admin_body_class .= ' tile_size_' . get_option('wc_pos_tile_size', 'square');
