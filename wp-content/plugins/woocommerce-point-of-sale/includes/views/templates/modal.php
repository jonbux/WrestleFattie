    ...
    <table class="product-fields-table" data-required="{{required}}">
        <tr>
            ...
            <td>
                ...
                {{/each}}
                {{/case}}
                {{#case "custom" "custom_text" break=true}}
                {{#each options}}
                <label class="pos_addon_label pos_addon_text">
                    {{getAddonsCustomField @index @../index ../type}}
                </label>
                {{/each}}
                {{/case}}
                {{#case "file_upload" break=true}}
                {{#each options}}
                <label class="pos_addon_label pos_addon_{{../type}}">
                    <input type="file" data-raw-price="{{price}}" placeholder=""
                           data-price="{{price}}"
                ...
                {{/each}}
                {{/case}}
                {{#case "select" "multiple_choice" break=true}}
            <td colspan="2">
                <select class="addon addon-select {{type}}_{{@index}}" {{#case "multiple_choice"}} multiple {{/case}}>
                    <option value="" selected><?php _e('Choose an option...', 'wc_point_of_sale'); ?></option>
                    {{#each options}}
...
<script type="text/template" id="tmpl-form-add-customer">
        ...
        <div class="woocommerce-custom-fields">
            <?php if (isset($checkout->checkout_fields['pos_acf'])) : ?>
                <?php foreach ($checkout->checkout_fields['pos_acf'] as $group) : ?>
                    ...
                    <span class="field_group_header"><?php echo $group['title']; ?></span>
                        ...
                        switch ($field['type']) {
                            case 'checkbox':
                                ...
                                foreach ($field['options'] as $value => $label) {
                                    ...
                                    printf('<label class="%5$s" for="%1$s-%2$s">%3$s%4$s</label>', $field['id'], $option_count, $input, esc_html($label), esc_attr($field['type']));
                                    $option_count++;
                                }
                                echo '</p>';
                            ...
                            case 'tel' :
                            case 'number' :
                                woocommerce_form_field($key, $field, '{{acf_fields.' . $field["name"] . '}}');
                                break;
                            case 'select' :
                                if (isset($field['multiple']) && $field['multiple'])
                                    $key .= '[]';
                                woocommerce_form_field($key, $field, is_array($field['value']) ? $field['value'][0] : $field['value']);
                                break;
                            case 'wysiwyg':
                                $field['type'] = 'textarea';
                                woocommerce_form_field($key, $field, '{{acf_fields.' . $field["name"] . '}}');
                                break;
                            case 'image':
                                echo '<p class="form-row" id="acf-field-'.$field["id"].'">';
                                echo '<label for="acf-field-'.$field["id"].'" class="">'.$field['label'].'</label>';
                                echo '<span class="woocommerce-input-wrapper"><input type="file" name="'.$field['id'].'" id="'.$field['id'].'"></span>';
                                echo '</p>';
                                break;
                            case 'color':
                                echo '<p class="form-row" id="acf-field-'.$field["id"].'">';
                                echo '<label for="acf-field-'.$field["id"].'" class="">'.$field['label'].'</label>';
                                echo '<span class="woocommerce-input-wrapper"><input type="color" name="'.$field['id'].'" id="'.$field['id'].'"></span>';
                                echo '</p>';
                                break;
                            default:
                                $field['class'] = isset($field['class']) ? implode(' ', $field['class']) : '';
                                echo '<div class="form-row acf-form-row">';
                                acf_render_fields(array($field));
                                echo '</div>';
                                break;
                        }
    ...
    <div id="pos_account_fund_fields" class="pos-customer-details-tab" style="display: block;">
        <div class="woocommerce-account-fund-fields">
            <p class="form-row notes" id="order_comments_field" data-priority="">
                <label for="order_comments" class="">Account funds available</span></label>
                <span id="account-funds-data" style="display:block;padding:10px"><strong><?php echo wc_price(0); ?></strong></span>
            </p>
        </div>
    </div>
    <?php endif; ?>
...
<script type="text/template" id="tmpl-order-info">
...
<script type="text/template" id="tmpl-signature">
    <div class="md-message-icon">
        <span class="dashicons dashicons-warning"></span>
    </div>
    <p><?php _e("A signature is required, please sign below on the solid line.", 'wc_point_of_sale'); ?></p>
    <div id="modal-signature-box-content">

    </div>
</script>
<script type="text/template" id="tmpl-confirm-save-order">
    <div class="md-message-icon">
        <span class="dashicons dashicons-warning"></span>
    </div>
    <p><?php _e("Are you sure you want to save order?", 'wc_point_of_sale'); ?></p>
</script>
