    ...
    if ($register_email_receipt) {
        ...
        switch ($order->get_status()) {
            case 'processing':
                ...
                break;
            case 'on-hold':
                $mail->emails['WC_Email_Customer_On_Hold_Order']->trigger($order_id);
                break;
            case 'completed':
                $mail->emails['WC_Email_Customer_Completed_Order']->trigger($order_id);
                break;
            case 'cancelled':
                $mail->emails['WC_Email_Cancelled_Order']->trigger($order_id);
                break;
            case 'refunded':
                $mail->emails['WC_Email_Customer_Refunded_Order']->trigger($order_id);
                break;
            case 'failed':
                $mail->emails['WC_Email_Failed_Order']->trigger($order_id);
                break;
            default:
                break;
        }
