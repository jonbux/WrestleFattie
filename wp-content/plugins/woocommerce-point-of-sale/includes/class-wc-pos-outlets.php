...
class WC_Pos_Outlets
{
    ...
    {
        add_action('admin_notices', array($this, 'outlet_notices'));
    }
    ...
    public function init_form_fields($country = '')
    {
        ...
        $shipping_data = array(
            'none' => __('Select shipping method', ' wc_point_of_sale')
        );
        $sm = array();
        $shipping_methods = pos_get_all_shipping_methods();
        foreach ($shipping_methods as $key => $value) {
            $sm[$key] = $value['title'];
        }
        $shipping_data = array_merge($shipping_data, $sm);
        $this->outlet_address_fields['default_shipping_method'] = array(
            'label' => __('Default shipping method', 'wo_point_of_sale'),
            'type' => 'select',
            'options' => $shipping_data
        );
    ...
    public function display_outlet_form($id = 0)
    {
        ...
        <div class="form-wrap">
        <h1 class="wp-heading-inline"><?php _e('Outlet Details', 'wc_point_of_sale'); ?></h1>
        <a href="#" class="page-title-action" id="use_store_address"><?php _e('Use Store Address', 'wc_point_of_sale') ?></a>
        ...
        foreach ($this->outlet_social_fields as $key => $field) {
            ...
            switch ($field['type']) {
                case "select" :
                    ...
                    break;
                default :
                    wc_pos_text_input(array('id' => '_outlet_' . $key, 'label' => $field['label'], 'value' => $value, 'description' => $field['description'], 'class' => 'social'));
                    break;
            }
        }
        ?>
    ...
    public function outlet_notices()
    {
        $action = isset($_REQUEST['action']) ? $_REQUEST['action'] : '';
        $message = '';
        if($action == 'edit-wc-pos-outlets'){
            $message = 2;
        }elseif ($action == 'add-wc-pos-outlets'){
            $message = 1;
        }
        ...
        if(!empty($message)){
            ...
            <div class="<?php echo $message['class']; ?>" style="margin-left: 0;max-width: 95%;">
                ...
                <p><a href="<?php echo esc_url(add_query_arg(array("page" => WC_POS()->id_outlets), admin_url('admin.php'))) ?>"><?php _e("&larr; Back to Outlets", "wc_point_of_sale"); ?></a></p>
            </div>
            <?php
        }
    }
