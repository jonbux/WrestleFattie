...
class WC_Pos_Registers
{
    ...
    public function init_form_fields()
    {
        ...
        $printers = array();
        $list = WC_POS_CPI()->star_cloudprnt_get_printer_list();
        if(count($list)){
            foreach ($list as $key => $printer)
                $printers[$key] = $printer['name'];
        }
        $receipt_print = array("show_print_dialogue" => "Show print dialogue");
        $other_print = array("no_printer" => "No printer selected");
        ...
        self::$register_detail_fields = array(
            ...
            'grid_template' => array(
                ...
                'type' => 'select',
                'value' => (isset($reg_data[0]['detail']['grid_template'])) ? $reg_data[0]['detail']['grid_template'] : 'categories',
            ...
            'default_customer' => array(
                'label' => __('Customer', 'wc_point_of_sale'),
                'type' => 'select',
            ...
            'default_shipping_method' => array(
                'label' => __('Shipping Method', 'wc_point_of_sale'),
                'type' => 'select',
            ...
            'disable_sale_prices' => array(
                'label' => __('Sale Prices', 'wc_point_of_sale'),
                'type' => 'select',
                'options' => array(0 => 'No', 1 => 'Yes'),
                'description' => __('Select whether you want the regular price to be displayed and used instead of sale price.', 'wc_point_of_sale'),
                'custom_attributes' => array()
            ),
            'dining_option_default' => array(
                'label' => __('Dining Option', 'wc_point_of_sale'),
                'type' => 'select',
                'options' => array('none' => 'None', 'eat_in' => 'Eat In', 'take_away' => 'Take Away', 'delivery' => 'Delivery'),
                'description' => __('Select the dining option you want to be used by default in the register.', 'wc_point_of_sale'),
                'custom_attributes' => array()
            )
        );
        $is_cloud_print = get_option('wc_pos_enable_cloud_print', 'disable');
        $is_paymentsense_enabled = in_array("wc_pos_paymentsense", get_option('pos_enabled_gateways', array()));
        if($is_paymentsense_enabled){
            self::$register_detail_fields["paymentsense_heading"] =  array(
                'type' => 'html',
                'html' => '<tr class="form-field"><th colspan="2"><h3>' . __("Paymentsense", "wc_point_of_sale") . '</h3></th></tr>',
            );
            self::$register_detail_fields["paymentsense_terminal"] =  array(
                'label' => __('Terminal', 'wc_point_of_sale'),
                'type' => 'select',
                'options' => $this->get_terminals(),
                'description' => __('Select the EMV terminal you want to use for this register.', 'wc_point_of_sale'),
                'custom_attributes' => array(),
                'default'   => 'none'
            );
            self::$register_detail_fields["paymentsense_eod_button"] =  array(
                'type' => 'html',
                'html' => '<tr class="form-field">
                            <th>' . __("EOD Report", "wc_point_of_sale") . '</th>
                            <td>
                                <button type="button" id="eod_report" class="button">'. __("Print EOD", "wc_point_of_sale") .'</button>
                                <p class="description">'. __("This will print a total of all EMV sales done for this terminal", "wc_point_of_sale") .'</p>
                            </td>
                        </tr>'
            );
        }
        ...
        if($is_cloud_print == "enable"){
            self::$register_end_of_sale_fields["receipt_printer"] = array(
                'label' => __('Receipt Printer', 'wc_point_of_sale'),
                'type' => 'select',
                'value' => (isset($reg_data[0]['detail']['receipt_printer'])) ? $reg_data[0]['detail']['receipt_printer'] : '',
                'options' => count($printers) ? array_merge($receipt_print, $printers) : $receipt_print,
                'description' => __('Select the printer that will use for receipt printing.', 'wc_point_of_sale'),
                'custom_attributes' => array()
            );
            self::$register_end_of_sale_fields["kitchen_printer"] = array(
                'label' => __('Kitchen Printer', 'wc_point_of_sale'),
                'type' => 'select',
                'value' => (isset($reg_data[0]['detail']['kitchen_printer'])) ? $reg_data[0]['detail']['kitchen_printer'] : '',
                'options' => count($printers) ? array_merge($other_print, $printers) : $other_print,
                'description' => __('Select the printer that will use for the kitchen.', 'wc_point_of_sale'),
                'custom_attributes' => array()
            );
            self::$register_end_of_sale_fields['bar_printer'] = array(
                'label' => __('Bar Printer', 'wc_point_of_sale'),
                'type' => 'select',
                'value' => (isset($reg_data[0]['detail']['bar_printer'])) ? $reg_data[0]['detail']['bar_printer'] : '',
                'options' => count($printers) ? array_merge($other_print, $printers) : $other_print,
                'description' => __('Select the printer that will use for the bar.', 'wc_point_of_sale'),
                'custom_attributes' => array()
            );
        }
    }
    ...
    public function save_register($redirect = true)
    {
        ...
        if(!preg_match('/^[a-z0-9 .\-]+$/i', trim($data['name']))){
            $_REQUEST['action'] = "validation-error";
            return false;
        }
        ...
        if ($redirect) {
            return wp_redirect(add_query_arg(array("page" => WC_POS()->id_registers, "message" => $message), 'admin.php'));
        }
    }
    ...
    function get_message($id = 0)
    {
        ...
        switch ($id) {
            case 1:
                $message['class'] = 'notice notice-success';
            ...
            case 3:
                ...
                break;
            case 4:
                $message['class'] = 'notice notice-error';
                $message['text'] = __('Validation error. Please use valid details.', 'wc_point_of_sale');
    ...
    public function send_report($content, $data)
    {
        ...
        $subject = $outlets_name[$data['outlet']] . ' ' . $data['closed'] . ' ' . $user->display_name . ' ' . html_entity_decode(get_woocommerce_currency_symbol()) . $formated_total;
    ...
    {
        $terminals = array();
        $payment_sense_api = new WC_Pos_PaymentSense_API();
        $pac_terminals = $payment_sense_api->pac_terminals_response();

        $terminals["none"] = __("None", "wc_point_of_sale");
        if(isset($pac_terminals['terminals']) && count($pac_terminals['terminals'])){
            foreach ($pac_terminals['terminals'] as $terminal){
                $terminals[$terminal['tpi']] = $terminal['tpi'];
            }
        }
    ...
    public function register_notices()
    {
        ...
        $message = '';
        if($action == 'edit-wc-pos-registers'){
            $message = 2;
        }elseif ($action == 'add-wc-pos-registers'){
            $message = 1;
        }elseif ($action == 'validation-error'){
            $message = 4;
        }
        ...
        if(!empty($message)){
            ...
            ?>
            <div class="<?php echo $message['class']; ?>" style="margin-left: 0;max-width: 95%;">
                <p><strong><?php echo $message['text']; ?></strong></p>
                <?php if(!isset($message['is_error'])): ?>
                    <p><a href="<?php echo esc_url(add_query_arg(array("page" => WC_POS()->id_registers), admin_url('admin.php'))) ?>"><?php _e("&larr; Back to Registers", "wc_point_of_sale"); ?></a></p>
                <?php endif; ?>
            </div>
            <?php
        }
